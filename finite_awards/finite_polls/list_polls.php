        ...
        $result=mysql_query($sql) or die('Unable to select from cell_poll');
          ...
          if ($num_record == 0){
               ...
               while (list($number, $category,$quest, $id) = mysql_fetch_row($result)) {
                   $total = $total + $number;
                  echo "<TR> <td>".$id." .... </td><td>".$category."</td><td><a href='view_poll_result.php?poll_id=".$id."'>View</a>|<a href='pie-chart-3d.php?poll_id=".$id."'>View Pie 3d </a>|<a href='list_table_polls.php?poll_id=".$id."'>View Table</a><td> <td>".$number."</td>";                
                echo "</TR>";   
