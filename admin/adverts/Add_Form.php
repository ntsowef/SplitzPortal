...
<div id="form">
		<table>
        	<form method="post" action="">
                ...
                <?php
					include ("connect.php");
					$g = mysql_query("select max(adv_id) from adverts");
        ...
        if(empty($wording))
		 {
		  ...
		  $status="False";
                  ...
                  if($status=='Ok')
		  {$date=date('Y'.'-'.'m'.'-'.'d');
		   $insert="INSERT into adverts(advert_wording,used_count,status,created_by,creation_date,active) values('$wording','0','$status1','$created_by',now(),'$active')";
		   echo $insert;
