...
$sql = "SELECT msisdn, category_code, reason, date_added ";
$sql.=" FROM wireles_nomination";
...
$sql = "SELECT msisdn, category_code, reason, date_added  ";
$sql.=" FROM wireles_nomination WHERE 1=1";
if( !empty($requestData['search']['value']) ) {   // if there is a search parameter, $requestData['search']['value'] contains search parameter
	$sql.=" AND ( msisdn LIKE '".$requestData['search']['value']."%' ";    
	$sql.=" OR category_code LIKE '".$requestData['search']['value']."%' ";
        
	$sql.=" OR reason LIKE '".$requestData['search']['value']."%' )";
}
$query=mysqli_query($conn, $sql) or die("nomination-data.php: get employees");
